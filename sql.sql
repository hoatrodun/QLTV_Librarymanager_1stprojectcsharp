...
BEGIN
	...
	IF EXISTS(SELECT * FROM inserted)
			BEGIN
				IF EXISTS(SELECT * FROM deleted)
					BEGIN
						SET @EVENT = CONCAT('ID BOOK DUOC UPDATE:', (SELECT SERIAL FROM INSERTED))
	...
	ELSE
			IF (SELECT COUNT(*) FROM  DELETED) > 0
				BEGIN
					SET @EVENT = CONCAT('ID BOOK DUOC XOA: ', (SELECT SERIAL FROM DELETED))
					EXEC PROC_INSERT_LOG_EVENT @EVENT
				END
END
GO
--DROP TRIGGER TRIG_UPDATE_LOG_BOOK
---------------ACCOUNT---------------
--DROP TRIGGER TRIG_UPDATE_LOG_ACCOUNT
...
BEGIN
	...
	IF EXISTS(SELECT * FROM inserted)
			BEGIN
				IF EXISTS(SELECT * FROM deleted)
					BEGIN
						SET @EVENT = CONCAT('UPDATE MAT KHAU CHO USER:', (SELECT USER_NAME FROM INSERTED))
						EXEC PROC_INSERT_LOG_EVENT @EVENT
					END
				ELSE
					BEGIN
						SET @EVENT = CONCAT('TAO USER:', (SELECT USER_NAME FROM INSERTED))
						EXEC PROC_INSERT_LOG_EVENT @EVENT
					END
			END
		ELSE
			IF (SELECT COUNT(*) FROM  DELETED) > 0
				BEGIN
					SET @EVENT = CONCAT('XOA USER: ', (SELECT USER_NAME FROM DELETED))
					EXEC PROC_INSERT_LOG_EVENT @EVENT
				END
END
GO
-----------------STUDENT---------------
DROP TRIGGER TRIG_UPDATE_LOG_STUDENT 
...
GO
-----------BORROW------------
--DROP TRIGGER TRIG_UPDATE_LOG_BORROW_DETAIL
	...
	IF (SELECT COUNT(*) FROM  INSERTED) > 0
		BEGIN
			...
			FETCH NEXT FROM CUR_BORROW_BOOK INTO @SERIAL, @ID
				...
				BEGIN
					SET @EVENT = CONCAT('TAO BORROW_CARD ID : ', (SELECT TOP 1 ID FROM INSERTED ), ' ID STUDENT:  ', @ID , ' ID BOOK :  ', @SERIAL)
					EXEC PROC_INSERT_LOG_EVENT @EVENT
					FETCH NEXT FROM CUR_BORROW_BOOK INTO @SERIAL, @ID
				END
...
END
go
DROP TRIGGER TRIG_UPDATE_LOG_BORROW 
...
BEGIN
	...
	IF EXISTS (SELECT *FROM  DELETED)
		BEGIN
					SET @EVENT = CONCAT('XOA BORROW_CARD ID : ', (SELECT TOP 1 ID FROM DELETED), 'STUDENT ID: ', (SELECT TOP 1 ID_STUDENT FROM DELETED))
					EXEC PROC_INSERT_LOG_EVENT @EVENT
		END
END
GO
--DROP PROCEDURE PROC_LOGIN_EVENT
CREATE PROC PROC_LOGIN_EVENT @USER_NAME VARCHAR(65)
AS
	BEGIN
		SET @USER_NAME = CONCAT('DANG NHAP VOI USERNAME: ', @USER_NAME)
		EXEC PROC_INSERT_LOG_EVENT @USER_NAME
	END

go
--DROP PROCEDURE PROC_LOGOUT_EVENT
CREATE PROC PROC_LOGOUT_EVENT @USER_NAME VARCHAR(65)
AS
	BEGIN
		SET @USER_NAME = CONCAT('LOGOUT VOI USERNAME: ', @USER_NAME)
		EXEC PROC_INSERT_LOG_EVENT @USER_NAME
	END
GO
--EXEC PROC_LOGIN_EVENT '12T'
...
UPDATE ACCOUNT 
	SET USER_NAME = 'dung1', PASSWORD = '1'
	WHERE USER_NAME = 'dung'

SELECT * FROM ACCOUNT
DELETE FROM ACCOUNT WHERE USER_NAME = 'dung'
INSERT INTO ACCOUNT VALUES('DUNGINSERT','123124')
SELECT * FROM ACCOUNT WHERE USER_NAME='DUNGINSERT'
GO
...
GO
--SELECT * FROM FUNCTION_LOGIN_ACCOUNT('dung1','1')

-----------------BOOK--------------
--SELECT * FROM BOOK

--INSERT INTO BOOK VALUES('SN13, N'Sách viết vội', 'x', 's ', 1, Null, 'df')
--INSERT INTO BOOK VALUES('S13', N'Sách viết vội', 'x', 's ', 1, Null, 'df')
--INSERT INTO BOOK VALUES('41', N'Sách viết vội', 'x', 's ', 1, Null, 'df')
--INSERT INTO BOOK VALUES('51', N'Sách viết vội', 'x', 's ', 1, Null, 'df')
--INSERT INTO BOOK VALUES('51', N'Sách viết vội', 'x', 's ', 1, Null, 'df')
--INSERT INTO BOOK VALUES('2', N'Sách viết vội', 'x', 's ', 1, Null, 'df')
--INSERT INTO BOOK VALUES('222', N'Sách viết vội', 'x', 's ', 1, Null, 'df')
--INSERT INTO BOOK VALUES('61', N'Sách viết vội', 'x', 's ', 1, Null, 'df')
--INSERT INTO BOOK VALUES('12N'Sách viết vội', 'x', 's ', 1, Null, 'df')
--INSERT INTO BOOK VALUES('SN1313', N'Sách viết vội', 'x', 's ', 1, Null, 'df')
...
--EXEC DBO.PROC_UPDATE_BOOK 'S92492', N'LAP TRINH HUONG DOI TUONG', 'LAP TRINH HUONG DOI TUONG', 'LAP TRINH HUONG DOI TUONG ', 1, Null, 'CÊCCE'
--select * from log order by TIME_CREATE
--END
...
--select * from book where SERIAL  = '2' And NAME like '%'
--CREATE FUNCTION FUNCTION_FIND_BOOK(@SERIAL NVARCHAR(15)
--SELECT * FROM FUNCTION_BOOK_WITH_TAG('123')
----------------STUDENT------------------
...
GO
--EXEC DBO.PROC_UPDATE_STUDENT 'S', N'S', 'S', 'S ', 1, Null, 'ƯERW'
...
GO
----------DELETE BORROW BOOK--------
DROP PROCEDURE PROC_DELETE_BORROW
...
GO
--exec PROC_INSERT_BORROW_DETAIL 57, 'S12412', 6, '1/124/2040', 1, '231'
go
